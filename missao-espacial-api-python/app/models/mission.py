...
class Mission(db.Model):
    ...
    release_date = db.Column(db.DateTime) #Datetime
    endpoint = db.Column(db.String)
    mission_state = db.Column(db.String)
    crew = db.Column(db.String)
    payload = db.Column(db.String)
    duration = db.Column(db.DateTime) #Datetime
    ...
    def list_id(self, mission_id):
        try:
            missions = db.session.query(Mission).filter(Mission.id == mission_id).all()
            missions_dict = [{'id': mission.id, 'name': mission.name, 'release_date': mission.release_date, 'endpoint': mission.endpoint, 'mission_state': mission.mission_state, 'crew': mission.crew, 'payload': mission.payload, 'duration': mission.duration, 'cost': mission.cost, 'status': mission.status} for mission in missions]
    ...
    def create_mission(self, name, release_date, endpoint, mission_state, crew, payload, duration, cost, status):
        try:
            ...
            print("Falha ao salvar a missão! ", error)  
    ...
    def update_mission(self, id, name, release_date, endpoint, mission_state, crew, payload, duration, cost, status):
        try:
            ...
            print("Falha ao atualizar a missão! ", error)
